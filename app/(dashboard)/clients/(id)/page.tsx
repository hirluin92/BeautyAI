...
export default async function ClientDetailPage({ params }: ClientDetailPageProps) {
  ...
  // Calculate stats
  const totalBookings = typedClient.visit_count || 0
  const totalSpent = Number(typedClient.total_spent || 0)
                ...
                <div className="flex items-center">
                  <div className="h-16 w-16 rounded-full bg-indigo-100 flex items-center justify-center mr-4">
                    <span className="text-indigo-600 font-bold text-xl">
                      {typedClient.full_name.charAt(0).toUpperCase()}
                    </span>
                  </div>
                  <div>
                    <h1 className="text-2xl font-bold text-gray-900">{typedClient.full_name}</h1>
                    <p className="text-gray-600">Cliente dal {formatDate(typedClient.created_at!)}</p>
                  </div>
                </div>
              </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center">
                    ...
                    <div>
                      ...
                      <a href={`tel:${typedClient.phone}`} className="text-gray-900 hover:text-indigo-600">
                        {formatPhone(typedClient.phone)}
                      </a>
                    </div>
                  </div>
                  ...
                  {typedClient.email && (
                    <div className="flex items-center">
                      ...
                      <div>
                        ...
                        <a href={`mailto:${typedClient.email}`} className="text-gray-900 hover:text-indigo-600">
                          {typedClient.email}
                        </a>
                      </div>
                    </div>
                  ...
                  {typedClient.whatsapp_phone && (
                    <div className="flex items-center">
                      <MessageSquare className="w-5 h-5 text-gray-400 mr-3" />
                      <div>
                        <p className="text-sm text-gray-600">WhatsApp</p>
                        <span className="text-gray-900">
                          {formatPhone(typedClient.whatsapp_phone)}
                        </span>
                      </div>
                    </div>
                  )}
              ...
              {typedClient.tags && typedClient.tags.length > 0 && (
                <div className="mt-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {typedClient.tags.map((tag, index) => (
                      <span
                        key={index}
            ...
            <div className="bg-white rounded-lg shadow">
              <div className="p-6 border-b border-gray-200">
                <div className="flex items-center justify-between">
                  ...
                  <Link
                    href={`/clients/${params.id}/bookings`}
            ...
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <div className="space-y-3">
                ...
                <button
                  ...
                  onClick={() => window.open(`tel:${typedClient.phone}`)}
                >
                  Chiama Cliente
                </button>
                {typedClient.email && (
                  <button
                    ...
                    onClick={() => window.open(`mailto:${typedClient.email}`)}
                  >
                    Invia Email
                  </button>
                )}
                {typedClient.whatsapp_phone && (
                  <button
                    ...
                    onClick={() => window.open(`https://wa.me/${typedClient.whatsapp_phone.replace(/\s/g, '')}`)}
                  >
                    WhatsApp
                  </button>
                )}
